...
import type { DecodedJWT, EphemeralKeyPair, ZKProof } from '~~/types/zkLoginTypes'
...
): Promise<{ url: string; nonce: string; randomness: string; maxEpoch: number }> {
    // Get current epoch
...
export async function fetchUserSalt(jwt: string): Promise<string> {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || `Salt service returned ${response.status}`)
        }
...
export async function fetchZKProof(
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json()
            throw new Error(errorData.error || `Prover service returned ${response.status}`)
        }
...
export function restoreSession(): {
    ...
    const ephemeralKeyPairStr = sessionStorage.getItem(STORAGE_KEYS.EPHEMERAL_KEY_PAIR)
