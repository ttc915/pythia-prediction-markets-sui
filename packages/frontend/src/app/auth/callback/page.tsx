...
export default function AuthCallback() {
    ...
    useEffect(() => {
        const handleCallback = async () => {
            try {
                ...
                if (!session.ephemeralKeyPair || !session.maxEpoch || !session.randomness || !session.nonce) {
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-sds-blue/10 to-sds-pink/10">
            <div className="w-full max-w-md rounded-lg bg-white p-8 shadow-xl dark:bg-slate-800">
                ...
                {status === 'error' && (
                    <div className="text-center">
                        ...
                        <button
                            onClick={() => router.push('/')}
                            className="rounded-lg bg-sds-blue px-6 py-2 text-white transition-colors hover:bg-sds-blue/90"
                        >
                            Return Home
                        </button>
                    </div>
