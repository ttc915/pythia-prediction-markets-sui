...
import { usePythia, usePythiaMarket, usePythiaProfile } from '~~/hooks/usePythia'
...
export function UserProfileExample() {
    ...
    return (
        <div>
            ...
            <p>Win Rate: {(parseInt(profile.total_wins) / parseInt(profile.total_bets) * 100).toFixed(2)}%</p>
            <p>Total Amount Bet: {profile.total_amount_bet} MIST</p>
            <p>Total Amount Won: {profile.total_amount_won} MIST</p>
            <p>Net Profit: {(BigInt(profile.total_amount_won) - BigInt(profile.total_amount_bet)).toString()} MIST</p>
        </div>
    )
}
    ...
    const handleFileDispute = async () => {
        try {
            ...
            const result = await fileDispute({
                marketId,
                positionId,
                reason: "The arbiters made an incorrect decision. Weather data shows no rain.",
 ...
 */
export function AcknowledgeDisputeWinExample({ marketId }: { marketId: string }) {
