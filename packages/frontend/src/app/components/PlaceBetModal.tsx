...
export default function PlaceBetModal({
    ...
    const { placeBet, getProtocolConfig, isLoading: isPythiaLoading } = usePythia()
    ...
    useEffect(() => {
        const fetchBalance = async () => {
            if (account?.address && open) {
                try {
                    const coinBalance = await suiClient.getBalance({ owner: account.address })
            ...
            <Dialog.Root open={open} onOpenChange={onOpenChange}>
                <Dialog.Content maxWidth="450px">
                    <Flex direction="column" gap="4" align="center" className="py-8">
                        ...
                        <Text size="5" weight="bold" className="text-center text-green-600 dark:text-green-400">
                            Bet Placed Successfully!
                        </Text>

                        <Text size="3" className="text-center text-slate-600 dark:text-slate-400">
                            You bet <span className="font-bold">{amount} SUI</span> on <span className={`font-bold ${betSide === 'yes' ? 'text-green-600' : 'text-red-600'}`}>{betSide?.toUpperCase()}</span>
                        </Text>
            ...
            <Dialog.Root open={open} onOpenChange={onOpenChange}>
                <Dialog.Content maxWidth="500px">
                    ...
                    <Flex direction="column" gap="5">
                        ...
                        <Flex gap="3">
                            <button
                                className={`flex-1 rounded-xl border-2 p-4 transition-all ${betSide === 'yes'
                            ...
                            >
                                <Flex direction="column" align="center" gap="1">
                                    <Text weight="bold" size="5" className="text-green-600 dark:text-green-400">
                                        YES
                                    </Text>
                            ...
                            <button
                                className={`flex-1 rounded-xl border-2 p-4 transition-all ${betSide === 'no'
                            ...
                            >
                                <Flex direction="column" align="center" gap="1">
                                    <Text weight="bold" size="5" className="text-red-600 dark:text-red-400">
                                        NO
                                    </Text>
                                    <Text size="2" className="text-slate-500">
                        ...
                        <div>
                            <div className="mb-2 flex justify-between">
                                <Text size="2" weight="bold">Amount (SUI)</Text>
                                {account && (
                                    <Text size="2" className="text-slate-500">
                        ...
                        {amount && betSide && (
                            <div className="rounded-lg bg-slate-50 p-4 dark:bg-slate-900">
                                ...
                                <Flex justify="between" align="center">
                                    ...
                                    <Text weight="bold">
                                        {((estimateWinnings(mistAmount, betSide === 'yes', market) / mistAmount - 1) * 100).toFixed(0)}%
                                    </Text>
                                </Flex>
                            </div>
                        )}
                        ...
                        {!account ? (
                            ...
                            <Button
                                size="4"
                                className={`w-full ${betSide === 'yes'
                                    ? 'bg-green-600 hover:bg-green-700'
                                    : betSide === 'no'
                                        ? 'bg-red-600 hover:bg-red-700'
                                        : 'bg-slate-900 dark:bg-white dark:text-slate-900'
                                    }`}
                                disabled={!betSide || !amount || isSubmitting || isPythiaLoading}
