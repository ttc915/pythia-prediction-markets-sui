...
export function PythiaProvider({ children }: { children: ReactNode }) {
    ...
    const getProtocolConfig = useCallback(async (): Promise<ProtocolConfig | null> => {
        try {
            ensureConfigured()
            const object = await suiClient.getObject({
                id: protocolConfigId,
    ...
    const createUserProfile = useCallback(async (): Promise<TransactionResult> => {
        try {
            setIsLoading(true)
            setError(null)
            ensureAccount()
        ...
        async (params: CreateMarketParams): Promise<CreateMarketResult> => {
            try {
                ...
                if (resolutionDeadline <= bettingEndTime) {
                    throw new Error(
                        'Resolution deadline must be after betting end time'
                    )
                }
                if (arbiters.length < arbiterThreshold) {
                    throw new Error(
                        'Number of arbiters must be >= arbiter threshold'
                    )
                }
    ...
    const fileDispute = useCallback(
        async (params: FileDisputeParams): Promise<FileDisputeResult> => {
            try {
                ...
                const { marketId, positionId, reason, proposedOutcome, bondAmount } = params
